...
//  ------ EXPRESS PUBLIC FOR IMAGES + EXPRESS SESSION FOR PASSPORT----- 
...
//  ------ index.ejs------ 
app.get('/', (req, res) => {
    res.render('index', { user: req.user, statuses: false, dataFromUser: false, sentimentScore: false });
});
...
//  ------ SEARCH FOR HASHES (used #beach for testing purposes)------ 
        ...
        T.get('search/tweets', { q: req.body.finder, count: 100 }, function (err, data, response) {
            for (var i = 0; i < data.statuses.length; i++) {
                if (data.statuses[i].geo !== null) {
                    ...
                    if (data.statuses[i].user.lang == 'en') {
                        ...
                        const userData = new TweetUserData({
                            ...
                            user_text: data.statuses[i].text
            ...
            .then(result2 => {
                ...
                TweetUserData.find().exec()
                    ...
                    .then(result3 => {
                        ...
                        var resultsent = []
                        var sumScore = 0;
                        result3.forEach((element2, index) => {
                            dataFromUser.push(element2);

                            //  ------ ANALYZE TEXT SENTIMENTS ----- 
                            resultsent.push(sentiment.analyze(element2.user_text));
                            sumScore += resultsent[index].score;

                        });
                        //  ------ SEND TO FRONT-END ----- 
                        res.render('index', { user: req.user, statuses: result2, dataFromUser: dataFromUser, sentimentScore: sumScore })
                    })
                    .then(lelele => {
